...
export default function UserRating({ userEmail, role, currentUserEmail, listingId, requireMessaged = false }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (ratings.length) setAvg(ratings.reduce((s, r) => s + r.score, 0) / ratings.length);
  ...
  const handleRate = async (score) => {
    ...
    if (myRating) {
      ...
      const created = await db.entities.Rating.create({ rater_email: currentUserEmail, rated_email: userEmail, role, score, listing_id: listingId || '' });
      setMyRating(created);
    }
    const ratings = await db.entities.Rating.filter({ rated_email: userEmail, role });
    setCount(ratings.length);
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2">
        ...
        {count > 0 && <span className="text-xs text-muted-foreground">({avg?.toFixed(1)} · {count})</span>}
      </div>
      <div className="flex items-center gap-2">
        <StarRating score={myRating?.score ?? avg ?? 0} onRate={canRate ? handleRate : undefined} size="sm" />
        {canRate && <span className="text-[10px] text-muted-foreground">{myRating ? 'Your rating' : 'Tap to rate'}</span>}
        {count === 0 && !canRate && !requireMessaged && <span className="text-xs text-muted-foreground">No ratings yet</span>}
      </div>
    </div>
  );
}
